...
export const ngsBaseSchema = z.object({
  ...
  player_display_name: z
    .string()
    .nullish()
    .describe("Player's full display name as shown in the feed"),
  player_position: nullableString.describe("Player's position: QB, RB, FB, HB, WR, or TE"),
  team_abbr: nullableString.describe(
    "Team abbreviation; null when the player is not currently on a team",
  ),
  ...
  player_short_name: nullableString.describe(
    "Player's short name (e.g. 'A.Rodgers'); null if unavailable",
  ),
});
...
export const ngsPassingSchema = ngsBaseSchema.extend({
  ...
  avg_air_yards_to_sticks: nullableFloat.describe(
    "Average air yards to the first-down sticks at the time of the throw",
  ),
  ...
  avg_air_distance: nullableFloat.describe(
    "Average air distance of pass attempts in yards; null if unavailable",
  ),
  max_air_distance: nullableFloat.describe(
    "Longest air distance of a pass attempt in yards; null if unavailable",
  ),
});

export const ngsReceivingSchema = ngsBaseSchema.extend({
  avg_cushion: nullableFloat.describe(
    "Average cushion in yards between the receiver and the nearest defender at the snap; null if unavailable",
  ),
  ...
  catch_percentage: nullableFloat.describe(
    "Catch percentage: receptions divided by targets (0-100)",
  ),
  ...
  avg_expected_yac: nullableFloat.describe(
    "Average expected yards after catch based on the NGS model; null if unavailable",
  ),
...
export const ngsRushingSchema = ngsBaseSchema.extend({
  efficiency: nullableFloat.describe(
    "Rush efficiency: actual rush yards over expected rush yards per attempt",
  ),
  ...
  avg_time_to_los: nullableFloat.describe(
    "Average time from snap to crossing the line of scrimmage in seconds",
  ),
  ...
  expected_rush_yards: nullableFloat.describe(
    "Model-predicted expected rush yards; null if unavailable",
  ),
  rush_yards_over_expected: nullableFloat.describe(
    "Rush yards over expected: actual minus expected; null if unavailable",
  ),
  rush_yards_over_expected_per_att: nullableFloat.describe(
    "Rush yards over expected per attempt; null if unavailable",
  ),
