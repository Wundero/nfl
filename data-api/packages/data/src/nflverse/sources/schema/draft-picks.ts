...
export const draftPicksSchema = z.object({
  ...
  cfb_player_id: nullableString.describe(
    "College Football Reference player identifier; null when empty",
  ),
  pfr_player_name: nullableString.describe(
    "Player name as listed on Pro-Football-Reference; null when empty",
  ),
  hof: boolFromBinary.describe(
    "True if the player is in the Pro Football Hall of Fame (0/null/undefined coerce to false; 'TRUE'/'FALSE' strings accepted)",
  ),
  position: nullableString.describe(
    "Position at time of draft: RB=Running Back, WR=Wide Receiver, T=Tackle, DE=Defensive End, TE=Tight End, DB=Defensive Back, G=Guard, QB=Quarterback, LB=Linebacker, C=Center, NT=Nose Tackle, P=Punter, DT=Defensive Tackle, K=Kicker, FB=Fullback, KR=Kick Returner, OL=Offensive Lineman, DL=Defensive Lineman, OLB=Outside Linebacker, CB=Cornerback, S=Safety, ILB=Inside Linebacker, LS=Long Snapper, OT=Offensive Tackle, SAF=Safety, FS=Free Safety, OG=Offensive Guard",
  ),
  category: nullableString.describe(
    "Broad position category: RB=Running Back, WR=Wide Receiver, OL=Offensive Line, DL=Defensive Line, TE=Tight End, DB=Defensive Back, QB=Quarterback, LB=Linebacker, P=Punter, K=Kicker, KR=Kick Returner, LS=Long Snapper, FS=Free Safety, OG=Offensive Guard",
  ),
  side: nullableString.describe(
    "Side of the ball: O=Offense, D=Defense, S=Special Teams; null when empty",
  ),
  college: nullableString.describe("College attended; null when empty"),
  age: nullableInt.describe("Age at the time of the draft"),
  to: nullableInt.describe(
    "Last season year in which the player played; null when empty or unparseable",
  ),
  ...
  w_av: nullableInt.describe(
    "Pro-Football-Reference weighted career Approximate Value (AV), emphasizing peak seasons; null when empty",
  ),
  car_av: z.coerce
    .string()
    .describe(
      "Pro-Football-Reference career Approximate Value (AV) as a raw string; appears empty in this feed — prefer w_av or dr_av",
    ),
  dr_av: nullableInt.describe(
    "Pro-Football-Reference draft Approximate Value (AV), the AV accrued during the player's rookie contract (draft-relative value); null when empty",
  ),
  games: nullableInt.describe("Career games played; null when empty"),
  pass_completions: nullableInt.describe(
    "College career pass completions (also present in the full player stats set); null when empty",
  ),
